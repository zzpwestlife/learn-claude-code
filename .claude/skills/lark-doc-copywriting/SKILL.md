...
metadata:
  ...
  assets:
    ...
    - lark_to_md.py         # 独立下载: URL → 本地 .md 文件
---
...
**逐行替换**：逐一比对每行，只对有变化的行调用 `replace_range`，跳过 `<image>` 行。
这样完全避免段落结构、空行、图片块的任何改动。
...
original_lines = open('/tmp/doc_original.md', encoding='utf-8').read().splitlines()
fixed_lines    = open('/tmp/doc_fixed.md',    encoding='utf-8').read().splitlines()
doc_id = open('/tmp/lark_doc_id.txt').read().strip()

success, fail = 0, 0
failed_items = []  # (i, orig_line, fixed_line)

def make_selection(line):
    """Build replace_range anchor covering the full line.
    ≤80 chars: use as-is. Longer: head[:40]...tail[-40:] ellipsis."""
    s = line.strip()
    if len(s) <= 80:
        return s
    head = s[:40]
...
def is_success(result):
    for src in (result.stdout, result.stderr):
        if src.strip().startswith('{'):
            try:
                resp = json.loads(src)
                if resp.get('data', {}).get('success') or resp.get('ok') is True:
                    return True
            except Exception:
                pass
    return False

def write_line(doc_id, selection, fixed_line):
...
for i, (orig, fixed) in enumerate(zip(original_lines, fixed_lines)):
    if orig == fixed:
        continue
    if orig.strip().startswith('<image '):
        continue  # image 行绝不替换 — 会导致图片块重复
    selection = make_selection(orig)
    result = write_line(doc_id, selection, fixed.strip())
    if is_success(result):
        success += 1
    else:
        fail += 1
        failed_items.append((i, orig, fixed))
        print(f"Line {i} FAILED: {result.stdout[:200] or result.stderr[:200]}")

print(f"\n完成: {success} 行成功, {fail} 行失败")
```
...
```python
retry_fail = 0
for i, orig, fixed in failed_items:
    selection = make_selection(orig)
    result = write_line(doc_id, selection, fixed.strip())
    if is_success(result):
        print(f"Line {i} 重试成功")
        ...
        retry_fail += 1
        print(f"Line {i} 重试仍失败: {result.stdout[:200] or result.stderr[:200]}")
...
| 约束 | 原因 |
|------|------|
| `<image>` 行绝不发送给 `replace_range` | 发送 image 行会让飞书创建新图片块，原图片仍存，导致图片重复 |
| 逐行替换，不做段落级替换 | 段落级替换改变 `\n`/`\n\n` 结构 → 产生多余空行；`<image>` 行混入 → 图片重复 |
| selection 用 `head[:40]...tail[-40:]` 覆盖整行 | 仅用 `[:80]` 作 anchor 时，超出部分不被替换而是追加到替换内容后，造成文本重叠 |
| 同时检查 stdout 和 stderr | lark-cli 成功写 stdout，错误写 stderr；只查 stdout 会把 stderr 错误误判为成功 |
| 已有 ` ``` ` 行跳过 | 已有代码块不重复包裹 |
...
| BEFORE | AFTER | 触发规则 |
|--------|-------|---------|
| `loading / error 三态` | `loading/error 三态` | 英文复合词斜杠去空格 |
| `看到空列表(还没有数据).` | `看到空列表 (还没有数据).` | 括号前补空格 |
| `数量;健康状态变绿` | `数量; 健康状态变绿` | 分号后补空格 |
| `模型列表(gpt-4o, gpt-3.5-turbo 等).` | `模型列表 (gpt-4o, gpt-3.5-turbo 等).` | 括号前补空格 |
| `DeepSeek(类型选 OpenAI 兼容` | `DeepSeek (类型选 OpenAI 兼容` | 括号前补空格 |
| `做.下一步` | `做. 下一步` | 句点后补空格（CJK→中文/大写） |
| `方案，继续` | `方案, 继续` | 全角逗号→半角 |

## Reusable Interface (R)

This skill must produce a reusable document-edit handoff.

Minimum contract:
- Record the doc identifier source, fetched markdown path, and whether code-candidate review was needed.
- Summarize exact write actions taken, skipped, or retried so a later session can resume safely.
- End with explicit success/failure counts instead of a narrative-only summary.

## Anti-Anchoring

- Do not broaden this workflow into general copy editing beyond punctuation, spacing, and code-block wrapping.
- Do not write back changes before the user confirms the summarized diff.
- Keep edits line-scoped and evidence-based; avoid "helpful" formatting changes outside the declared contract.
