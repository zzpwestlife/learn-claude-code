...
```bash
# lark-cli docs +fetch 返回 {"data": {"markdown": "..."}}
...
# 无图片时退化为单段；若单段超大导致 replace_range 失败，fail 计数器捕获并触发重试提示
...
| `selection` = 首行前50字符 + `...` + 末行后50字符 | lark `replace_range` 模糊匹配定位段落；若匹配失败则 `success: false`，由 `fail` 计数器捕获 |
