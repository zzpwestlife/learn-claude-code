...
5.  **自动化闭环 (Automation Loop)**：内置 **Skill Audit** 机制。
    *   `/audit-skills`：智能分析日志，自动发现工作流缺口并推荐新技能。
6.  **双重人格架构 (Dual Persona)**：独创的 **Builder (Opus)** vs **Critic (Codex)** 协作模式。Opus 负责从 0 到 1 的创造与文档，Codex 负责严格的代码审查与安全审计 (`.claude/skills/code-review/SKILL.md`)。
7.  **智能体技能库**：内置 Python 驱动的高级技能（如 `changelog-generator`），位于 `.claude/skills/` 目录，提供自动变更日志等能力（仅依赖系统 Python，无需额外配置）。
...
| `code-review` (Skill) | **全能审查员**：统一处理代码审查的请求、执行与接收。 | 提交代码前，尤其是 Pull Request 前 |
